...
def upload():
    ...
    if action == 'config':
        ...
        if fieldName in request.files:
            ...
            uploader = Uploader(field, config, ueditor_server.static_folder)
        ...
        if fieldName in request.form:
            ...
            uploader = Uploader(field, config, ueditor_server.static_folder, 'base64')
            result = uploader.getFileInfo()
        else:
            result['state'] = '上传接口出错'
        ...
        for imgurl in source:
            uploader = Uploader(imgurl, config, ueditor_server.static_folder, 'remote')
